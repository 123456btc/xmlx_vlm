...
logger = logging.getLogger("xmlx_vlm.diffusion_generate")
...
# Markers that must survive the diffusion output detokenizer so downstream
# parsers (tool calls, thinking separation) can consume them.
_DIFFUSION_PRESERVED_MARKERS = {
    "<|tool_call>",
    "<tool_call|>",
    "<|tool_response>",
    "<tool_response|>",
...
def _preserve_diffusion_markers(skip_ids, tokenizer):
    """Return ``skip_ids`` with tool/channel markers removed."""
    if not skip_ids:
        return skip_ids
    preserved = set()
    for marker in _DIFFUSION_PRESERVED_MARKERS:
        try:
...
def stream_diffusion_generate(
    ...
    *,
    ...
    prompt_tokens = len(full_input_ids_list)
    ...
    # Prompt-cache reuse: skip the common prefix from previous turns.
    reused_prefix_len = 0
    kv_cache = None
    if (
        prompt_cache_state is not None
        and prompt_cache_state.cache is not None
        and prompt_cache_state.token_ids is not None
    ):
        prefix_len = prompt_cache_state.find_prefix_length(full_input_ids_list)
        if 0 < prefix_len < len(full_input_ids_list):
            can_reuse = all(
                getattr(c, "is_trimmable", lambda: False)()
                for c in prompt_cache_state.cache
            )
            if can_reuse:
                kv_cache = prompt_cache_state.cache
                for c in kv_cache:
                    if getattr(c, "offset", 0) > prefix_len:
    ...
    use_static_cache = (
        ...
        and static_cache_length > prompt_length
    ...
    if use_static_cache:
        ...
        if kv_cache is None:
            ...
            # Reusing a per-turn prompt cache. The cache already holds the
            # prefix; the decoder mask must cover prefix + new suffix.
            if has_padding:
                prefix_mask = mx.ones(
                    (batch_size, reused_prefix_len), dtype=mx.bool_
                )
                decoder_attention_mask = mx.concatenate(
                    [prefix_mask, attention_mask], axis=-1
                )
            else:
                decoder_attention_mask = None
            cached_sequence_length = reused_prefix_len + prompt_length
    ...
    with mx.stream(generation_stream):
        ...
        while generated_tokens < max_new_tokens:
            ...
            for token_id in current_canvas[0].tolist():
                ...
                generated_tokens += 1
                generated_token_ids.append(last_token)
    ...
    # Save KV cache for the next turn so only the new suffix needs prefill.
    if prompt_cache_state is not None:
        all_ids = full_input_ids_list + generated_token_ids
        prompt_cache_state.update(all_ids, kv_cache)
...
def stream_diffusion_generate_from_kwargs(
    ...
    # Preserve tool-call / channel markers for downstream parsers.
    skip_special_token_ids = _preserve_diffusion_markers(
        skip_special_token_ids, tokenizer
    )
