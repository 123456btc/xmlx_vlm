...
from .utils import BASE_DIR, draw_point, update_navigation_history
...
# Unified default: follow XMLX_VLM_MODEL, fallback to DEFAULT_MODEL (SSOT).
# Use XMLX_VLM_COMPUTER_PLANNER_MODEL / XMLX_VLM_COMPUTER_GUI_MODEL to override.
PLANNER_MODEL = os.environ.get(
    "XMLX_VLM_COMPUTER_PLANNER_MODEL",
...
# Whisper is used for speech recognition; diffusiongemma cannot do this.
WHISPER_MODEL = os.environ.get(
    "XMLX_VLM_COMPUTER_WHISPER_MODEL",
...
def highlight_click(x, y, duration=0.2):
    ...
    for i in range(steps):
        ...
        _get_mouse().position = (x + radius * math.cos(angle), y + radius * math.sin(angle))
        time.sleep(duration / steps)
    # Return to center
    _get_mouse().position = (x, y)
...
def run_planner(model, processor, screenshot, system_prompt, query, past_actions=[]):
    ...
    # xmlx_vlm.generate returns a GenerationResult dataclass in recent versions.
...
def main():
    ...
    print(f"Using GUI model: {GUI_MODEL}")
    print("Press Ctrl+C to quit")
    model, processor = load(PLANNER_MODEL)
    ...
    past_actions = []
    counter = 0
    play_audio(str(AUDIO_DIR / "ask_voice.wav"))
    input("Press Enter to start listening...")
    query = listen(mlx_whisper, mic, r)
    # Play audio confirmation sound
    play_audio(str(AUDIO_DIR / "ok.wav"))
    ...
    try:
        while True:
            ...
            if counter % 3 == 0 or any(
                ...
                play_audio(str(AUDIO_DIR / "ask_voice_2.wav"))
                input("Press Enter to start listening...")
                query = listen(mlx_whisper, mic, r)
                past_actions = []
