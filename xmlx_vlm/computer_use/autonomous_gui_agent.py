...
# Defaults follow XMLX_VLM_MODEL, falling back to DEFAULT_MODEL (SSOT).
...
def process_command(model, processor, gui_agent, gui_processor, query, past_actions=[]):
    ...
    if any(
        ...
        play_audio(str(AUDIO_DIR / "task_completed.wav"))
        print("Task completed successfully!")
        screenshot = ImageGrab.grab()
        screenshot = screenshot.resize((1512, 982))
...
def main():
    ...
    past_actions = []
    play_audio(str(AUDIO_DIR / "ask.wav"))
    ...
    while True:
        ...
        if counter % 3 == 0 or any(
            ...
            play_audio(str(AUDIO_DIR / "ask_2.wav"))
            query = Prompt.ask("\nWhat would you like me to do?")
            past_actions = []
