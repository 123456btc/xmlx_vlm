...
def get_model_and_args(config: dict):
    ...
    for pkg in ("xmlx_vlm.models", "xmlx_vlm.speculative.drafters"):
        try:
...
```
python -m xmlx_vlm.convert --hf-path <local_dir> --mlx-path <mlx_dir>
```
        """
...
def upload_to_hub(path: str, upload_repo: str):
    ...
    if hf_path is not None:
        ...
        using xmlx-vlm version **{__version__}**.
    ...
    card.text = dedent(
        ...
        ```bash
        pip install -U xmlx-vlm
        ```

        ```bash
        python -m xmlx_vlm.generate --model {upload_repo} --max-tokens 100 --temperature 0.0 --prompt "Describe this image." --image <path_to_image>
        ```
        """
    )
    card.save(card_path)
