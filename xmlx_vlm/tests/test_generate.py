...
from xmlx_vlm import apc as apc_module
...
from xmlx_vlm.utils import ThinkingBudgetCriteria

generate_module = sys.modules["xmlx_vlm.generate"]
...
class MockModel:
    ...
    def make_cache(self):
        from xmlx_vlm.models import cache
...
class TestGenerationBatch:
    ...
    def _capture(value, B):
        from xmlx_vlm.generate import PromptProcessingBatch
...
class TestBatchGenerate:
    ...
    def test_text_only_batch(self, mock_generate_batch, mock_model, mock_processor):
        ...
        from xmlx_vlm.generate import batch_generate
    ...
    @patch.object(generate_module, "_generate_batch")
    @patch("xmlx_vlm.utils.process_image")
...
def test_generate_cli_smoke(capsys):
    ...
    generate_module = importlib.import_module("xmlx_vlm.generate")
...
def test_parse_arguments_defaults_thinking_tokens(monkeypatch):
    monkeypatch.setattr(sys, "argv", ["xmlx_vlm.generate"])
...
def test_cached_prefix_rope_failure_falls_back_to_cold(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="xmlx_vlm.generate"):
