...
def test_load_passes_revision():
    ...
    with (
        patch("xmlx_vlm.utils.get_model_path") as mock_get_model_path,
        patch(
            "xmlx_vlm.utils.load_model",
            return_value=model_mock,
        ) as mock_load_model,
        patch(
            "xmlx_vlm.utils.load_processor",
            return_value=processor_mock,
        ) as mock_load_processor,
        patch("xmlx_vlm.utils.load_image_processor", return_value=None),
    ):
...
class TestLoadImage:
    ...
    def test_http_url_input(self):
        ...
        with patch("xmlx_vlm.utils.requests.get", return_value=mock_response):
            img = load_image("https://example.com/image.png")
            assert img.mode == "RGB"

    def test_invalid_url_raises(self):
        with patch(
            "xmlx_vlm.utils.requests.get",
