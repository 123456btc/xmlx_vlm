...
Usage:
    # Python API
    from xmlx_vlm.models.sam3.generate import Sam3Predictor, Sam3VideoPredictor

    predictor = Sam3Predictor(model, processor)
    result = predictor.predict(image, text_prompt="a dog")

    # Video tracking CLI
    python -m xmlx_vlm.models.sam3.generate --video input.mp4 --prompt "a car"
"""
...
def track_video(
    ...
    from xmlx_vlm.generate import wired_limit
    from xmlx_vlm.models.sam3.processing_sam3 import Sam3Processor
    from xmlx_vlm.utils import get_model_path, load_model
...
def track_video_realtime(
    ...
    with wired_limit(model):
        ...
        if annotator_name:
            ...
            from xmlx_vlm.models.sam3.annotators import (
                BoxAnnotator,
                LabelAnnotator,
                MaskAnnotator,
            )
...
def build_annotator(name: str, opacity: float = 0.6, contour_thickness: int = 2):
    ...
    """
    from xmlx_vlm.models.sam3 import annotators as ann_module
...
def main():
  ...
  python -m xmlx_vlm.models.sam3.generate --task segment --image photo.jpg --prompt "a cat"

  # Instance segmentation with boxes overlaid
  python -m xmlx_vlm.models.sam3.generate --task segment --image photo.jpg --prompt "a cat" --show-boxes

  # Box-guided segmentation
  python -m xmlx_vlm.models.sam3.generate --task segment --image photo.jpg --prompt "a cat" --boxes "10,50,300,400"
  ...
  python -m xmlx_vlm.models.sam3.generate --task realtime --prompt "a person"

  # Real-time webcam with background swap
  python -m xmlx_vlm.models.sam3.generate --task realtime --prompt "a person" --bg-image beach.jpg
""",
    )
    parser.add_argument(
        "--task",
