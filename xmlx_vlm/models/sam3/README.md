...
from xmlx_vlm.utils import load_model, get_model_path
from xmlx_vlm.models.sam3.generate import Sam3Predictor
from xmlx_vlm.models.sam3.processing_sam3 import Sam3Processor
...
```bash
python -m xmlx_vlm.models.sam3.generate --task <task> --prompt "..." ...
```

### Object Detection (boxes only)

```bash
python -m xmlx_vlm.models.sam3.generate --task detect --image photo.jpg --prompt "a cat"
```

### Instance Segmentation (masks only, default)

```bash
python -m xmlx_vlm.models.sam3.generate --image photo.jpg --prompt "a cat"

# With boxes overlaid
python -m xmlx_vlm.models.sam3.generate --image photo.jpg --prompt "a cat" --show-boxes

# Box-guided segmentation
python -m xmlx_vlm.models.sam3.generate --image photo.jpg --prompt "a cat" --boxes "0,50,350,480"

# Multiple box prompts
python -m xmlx_vlm.models.sam3.generate --image photo.jpg --prompt "a cat" --boxes "0,50,350,480;300,20,640,375"
```

### Video Tracking (to file)

```bash
python -m xmlx_vlm.models.sam3.generate --task track --video input.mp4 --prompt "a car"

# Track only objects in a region
python -m xmlx_vlm.models.sam3.generate --task track --video input.mp4 --prompt "a car" --boxes "200,100,1200,900"

# Faster with lower resolution
python -m xmlx_vlm.models.sam3.generate --task track --video input.mp4 --prompt "a car" --resolution 336
```
...
```bash
python -m xmlx_vlm.models.sam3.generate --task realtime --prompt "a person" --resolution 224

# Multiple objects
python -m xmlx_vlm.models.sam3.generate --task realtime --prompt "a person" "a phone" --resolution 224

# With boxes and labels
python -m xmlx_vlm.models.sam3.generate --task realtime --prompt "a cup" --resolution 224 --show-boxes
```
...
```bash
python -m xmlx_vlm.models.sam3.generate --task realtime --prompt "a person" --resolution 224 --bg-image beach.jpg
```
...
```python
import cv2
from xmlx_vlm.models.sam3.generate import Sam3VideoPredictor
