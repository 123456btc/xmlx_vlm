...
```python
from xmlx_vlm.models.sam3d_body.generate import SAM3DPredictor
...
```python
from xmlx_vlm.models.sam3d_body.estimator import write_obj
...
```python
import cv2
from xmlx_vlm.models.sam3d_body.overlay import (
...
```bash
python -m xmlx_vlm.models.sam3d_body.video --input pitch.mp4 --output pitch_overlay.mp4
```
...
python -m xmlx_vlm.models.sam3d_body.generate --image photo.jpg --output mesh.obj

# With bounding box
python -m xmlx_vlm.models.sam3d_body.generate --image photo.jpg --bbox 100,50,400,500 --output mesh.obj

# Custom weights directory
python -m xmlx_vlm.models.sam3d_body.generate --image photo.jpg --weights /path/to/weights/ --output mesh.obj

# Save 3D keypoints alongside mesh
python -m xmlx_vlm.models.sam3d_body.generate --image photo.jpg --output mesh.obj --save-keypoints
```
...
```bash
python -m xmlx_vlm.models.sam3d_body.convert_weights \
...
├── generate.py            # SAM3DPredictor — xmlx-vlm from_pretrained/predict API
...
├── vision.py              # VisionModel stub (wraps backbone for xmlx-vlm compat)
└── language.py            # LanguageModel stub (raises NotImplementedError)
```

### xmlx-vlm Integration

This package follows xmlx-vlm model conventions:
...
- `sanitize()` static method for xmlx-vlm's weight loading path
- `model_type: "sam3d_body"` in config, intended for xmlx-vlm's `MODEL_REMAPPING`
...
The original SAM 3D Body model weights are released by Meta. The MLX port code follows the same license as [xmlx-vlm](https://github.com/Blaizzy/mlx-vlm). See the upstream repository for model weight license details.
